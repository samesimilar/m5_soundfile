...
static int m5_wave_isextended(const t_soundfile *sf)
{
    return sf->sf_bytespersample == 4 || sf->sf_bytespersample == 8;
}

    /** read first chunk, returns filled chunk and offset on success or -1 */
static off_t m5_wave_firstchunk(const t_soundfile *sf, t_chunk *chunk)
{
    if (m5_fd_read(sf->sf_fd, WAVEHEADSIZE, (char *)chunk,
...
static off_t m5_wave_nextchunk(const t_soundfile *sf, off_t offset, t_chunk *chunk)
{
    uint32_t chunksize = m5_swap4(chunk->c_size, m5_sys_isbigendian());
    off_t seekto = offset + WAVECHUNKSIZE + chunksize;
    if (seekto & 1) /* pad up to even number of bytes */
        seekto++;
    if (m5_fd_read(sf->sf_fd, seekto, (char *)chunk,
        WAVECHUNKSIZE) < WAVECHUNKSIZE)
        return -1;
    return seekto;
}
...
static void m5_wave_postchunk(const t_chunk *chunk, int swap)
{
    post("%.4s %d", chunk->c_id, swap4s(chunk->c_size, swap));
}

    /** post head info for debugging */
static void m5_wave_posthead(const t_head *head, int swap)
{
...
static void m5_wave_postformat(const t_formatchunk *format, int swap)
{
...
static void m5_wave_postfact(const t_factchunk *fact, int swap)
{
...
static int m5_wave_isheader(const char *buf, size_t size)
{
    if (size < 4) return 0;
    return !strncmp(buf, "RIFF", 4);
}

static int m5_wave_readheader(t_soundfile *sf)
{
    int nchannels = 1, bytespersample = 2, samplerate = 44100, bigendian = 0,
        swap = (bigendian != m5_sys_isbigendian()), formatfound = 1;
    ...
    t_chunk *chunk = &buf.b_chunk;
        ...
        /* file header */
    if (m5_fd_read(sf->sf_fd, 0, buf.b_c, headersize) < headersize)
        return 0;
    ...
    if ((headersize = m5_wave_firstchunk(sf, chunk)) == -1)
        return 0;
    while (1)
    {
        uint32_t chunksize = m5_swap4(chunk->c_size, swap);
        ...
        if (!strncmp(chunk->c_id, "fmt ", 4))
        {
            ...
            if (m5_fd_read(sf->sf_fd, headersize + 8,
                    buf.b_c + 8, chunksize) < chunksize)
                return 0;
#ifdef DEBUG_SOUNDFILE
            wave_postformat(format, swap);
#endif
            nchannels = m5_swap2(format->fc_nchannels, swap);
            samplerate = m5_swap4(format->fc_samplerate, swap);
            formattag = m5_swap2(format->fc_fmttag, swap);
            ...
            switch (formattag)
            {
                ...
                case WAVE_FORMAT_EXT:
                    formattag = m5_swap2(*(uint16_t *)&format->fc_subformat, swap);
            ...
            bytespersample = m5_swap2(format->fc_bitspersample, swap) / 8;
        ...
        {
                /* sound data chunk */
            bytelimit = m5_swap4(chunk->c_size, swap);
...
#endif
        if ((headersize = m5_wave_nextchunk(sf, headersize, chunk)) == -1)
            return 0;
    }
    if (!formatfound)
    {
...
static int m5_wave_writeheader(t_soundfile *sf, size_t nframes)
{
    int isextended = m5_wave_isextended(sf), swap = m5_soundfile_needsbyteswap(sf);
    ...
    t_formatchunk format = {
        "fmt ", m5_swap4(16, swap),
        WAVE_FORMAT_PCM,                                  /* format tag      */
        m5_swap2((uint16_t)sf->sf_nchannels, swap),          /* channels        */
        m5_swap4((uint32_t)sf->sf_samplerate, swap),         /* sample rate     */
        m5_swap4((uint32_t)(sf->sf_samplerate *              /* bytes per sec   */
                         sf->sf_bytesperframe), swap),
        m5_swap2((uint16_t)sf->sf_bytesperframe, swap),      /* block align     */
        m5_swap2((uint16_t)sf->sf_bytespersample * 8, swap), /* bits per sample */
        0                                                 /* extended format */
    };
    t_chunk data = {"data", m5_swap4((uint32_t)datasize, swap)};
    ...
    if (sf->sf_bytespersample == 4 || sf->sf_bytespersample == 8)
        format.fc_fmttag = m5_swap2(WAVE_FORMAT_FLOAT, swap);
    if (isextended)
    {
        format.fc_extsize = m5_swap2(WAVE_EXT_SIZE - 2, swap); /* - fmttag */
        ...
        format.fc_fmttag = m5_swap2(WAVE_FORMAT_EXT, swap);
        formatsize += WAVE_EXT_SIZE;
    }
    format.fc_size = m5_swap4(formatsize - 8, swap);
    ...
    {
        t_factchunk fact = {
            "fact", m5_swap4(4, swap),
            m5_swap4((uint32_t)(sf->sf_nchannels * nframes), swap)
        };
    ...
    {
            /* add pad byte */
        data.c_size = m5_swap4((uint32_t)datasize + 1, swap);
    }
    memcpy(buf + headersize, &data, WAVECHUNKSIZE);
    headersize += WAVECHUNKSIZE;

        /* update file header chunk size (- chunk header) */
    head.h_size = m5_swap4s((int32_t)(datasize + headersize - 8), swap);
    ...
    byteswritten = m5_fd_write(sf->sf_fd, 0, buf, headersize);
...
static int m5_wave_updateheader(t_soundfile *sf, size_t nframes)
{
    int isextended = m5_wave_isextended(sf), swap = m5_soundfile_needsbyteswap(sf);
    ...
    if (isextended)
    {
        ...
        uinttmp = m5_swap4((uint32_t)(nframes * sf->sf_nchannels), swap);
        if (m5_fd_write(sf->sf_fd, headersize + 8, &uinttmp, 4) < 4)
            return 0;
        headersize += WAVEFACTSIZE;
    }
    ...
    datasize += padbyte;
    uinttmp = m5_swap4((uint32_t)datasize, swap);
    if (m5_fd_write(sf->sf_fd, headersize + 4, &uinttmp, 4) < 4)
        return 0;
    headersize += WAVECHUNKSIZE;
    ...
    {
        uinttmp = 0;
        if (m5_fd_write(sf->sf_fd, headersize + datasize - 1, &uinttmp, 1) < 1)
            return 0;
    }

        /* file header chunk size (- chunk header) */
    uinttmp = m5_swap4((uint32_t)(headersize + datasize - 8), swap);
    if (m5_fd_write(sf->sf_fd, 4, &uinttmp, 4) < 4)
        return 0;
...
static int m5_wave_hasextension(const char *filename, size_t size)
{
...
static int m5_wave_addextension(char *filename, size_t size)
{
    int len = strnlen(filename, size);
    if (len + 4 >= size)
        return 0;
    ...
    /* force little endian */
static int m5_wave_endianness(int endianness, int bytespersample)
{
    return 0;
}
...
t_soundfile_type wave = {
    ...
    m5_wave_readheader,
    m5_wave_writeheader,
    m5_wave_updateheader,
    m5_wave_hasextension,
    m5_wave_addextension,
    m5_wave_endianness
};

void m5_soundfile_wave_setup( void)
{
    m5_soundfile_addtype(&wave);
}
